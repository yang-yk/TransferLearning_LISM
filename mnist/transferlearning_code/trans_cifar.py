...
def save_img(imgs,path):
    ...
    for i in range(imgs.shape[0]): 
        ...
        if i%50==0:
           print('saving '+path+':',i)
        plt.imshow(np.transpose(imgs[i,:,:,0]),cmap='gray',interpolation="bicubic")
...
fine_tfrecords_trainpath='../transferlearning_dataset/cifar_train_data.tfrecords'
fine_tfrecords_testpath='../transferlearning_dataset/cifar_test_data.tfrecords'
...
#train_imgs=OCN.build(train_data_batch,Option.channel,is_training=False,fine_training=True)
#test_imgs=OCN.build(test_data_batch,Option.channel,is_training=False,fine_training=False)
...
model_path='../training_experiment/2019-10-22-12:11:20/model/'
